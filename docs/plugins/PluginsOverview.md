...
- The internal *plugins* directory located within the CSE installation path at `acme/plugins`. This directory and its subdirectories are used for plugins that are bundled with the CSE. You should **not** modify or add plugins to this directory, as they may be overwritten during CSE updates.
